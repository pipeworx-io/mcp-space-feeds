 ...
 * Space Feeds MCP.
 *
 * Space & spaceflight — NASA/ESA, launch providers, astronomy and the space industry.
...
const FEEDS: Record<string, FeedDef> = {
  ...
  spacenews: { title: "SpaceNews", url: "https://spacenews.com/feed/", category: "space", source: "SpaceNews", description: "Space industry & policy." },
  'nasa-breaking': { title: "NASA", url: "https://www.nasa.gov/feed/", category: "space", source: "NASA", description: "NASA mission & science news." },
  'planetary-society': { title: "Planetary Society", url: "https://www.planetary.org/rss/articles", category: "space", source: "The Planetary Society", description: "Planetary science & space advocacy." },
};

const tools: McpToolExport['tools'] = [
  {
    name: 'list_feeds',
    description: 'List the curated space & spaceflight feeds (id, title, category, source). Optionally filter by category (space) or keyword. Pass an id to read_feed.',
    inputSchema: {
      type: 'object',
      properties: {
        category: { type: 'string', description: 'Filter by category: space.' },
  ...
  {
    name: 'read_feed',
    description: 'Read a curated space & spaceflight feed by its id (from list_feeds). Returns normalized items (title, link, published, summary). Optionally filter items by keyword.',
    inputSchema: {
      type: 'object',
      properties: {
